...
class PiwikPROProvider extends BaseProvider {
   ...
   * @returns {void|Array}
   */
  handleCustom(_url, params) {
    const results = [];
    const revenue = params.get('revenue');
    const _cvar = params.get('_cvar');
    const cvar = params.get('cvar');
    const ec_items = params.get('ec_items');
    let requestType = 'Page View';
    ...
    if (_cvar) {
      try {
        const customVars = JSON.parse(_cvar);
    ...
    if (cvar) {
      try {
        const customVars = JSON.parse(cvar);
        /* istanbul ignore else: do nothing when it's null/empty */
        if (typeof customVars === 'object' && customVars) {
          Object.entries(customVars).forEach(([key, [name, value]]) => {
            results.push(
    ...
    if (ec_items) {
      try {
        const products = JSON.parse(ec_items);
        /* istanbul ignore else: do nothing when it's null/empty */
          ...
          products.forEach(([sku, name, category, price, qty], i) => {
            ...
            results.push(
              ...
              },
              {
                ...
                value:
                  typeof category === 'object' && category.length
                    ? category.join(', ')
                    : category,
                group: 'ecommerce',
              },
