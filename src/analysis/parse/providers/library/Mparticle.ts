...
class MparticleProvider extends BaseProvider {
  ...
  handleCustom(url, params) {
    ...
    const clientCodematches = url.pathname.match(clientCodeRe);
    ...
    if (etType) {
      ...
      const etValue = etDict[etType] ? etDict[etType] : etType;
      results.push({
        key: 'etParsed',
    ...
    if (dataType) {
      ...
      const dataTypeValue = dataTypeDict[dataType]
    ...
    const eventType = params.get('n') || params.get('events[0].event_type');
    const eventDict = {
      pageView: 'Page View',
      '1': 'Session Start',
      '2': 'Session End',
    ...
    const eventTypeValue = eventDict[eventType]
      ? eventDict[eventType]
      : eventType;
    results.push({
    ...
    output.forEach((e) => {
      const idValue = e.toString().split(',')[1];
      const typeValue = e.toString().split(',')[3];
      results.push({
        key: identityTypeDict[typeValue]
          ? identityTypeDict[typeValue]
          : typeValue,
