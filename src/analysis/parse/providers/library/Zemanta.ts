...
class ZemantaProvider extends BaseProvider {
  ...
  handleCustom(_url, _params) {
    const results = [];
    const legacyPixel = /^\/p\//.test(_url);
    const pixelVersion = _url.pathname.match(/^\/(v\d+)\//i);
    const pixelID = _url.pathname.match(/\/p\/(?:js\/)?(\d+)\/\//i);
    const eventType = _url.pathname.match(/\/js\/\d+\/([^/]+)?/i);
...
export default ZemantaProvider;
