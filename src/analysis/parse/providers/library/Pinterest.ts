...
class PinterestProvider extends BaseProvider {
  ...
  handleCustom(_url, params) {
    const results = [];
    const event = params.get('event') || 'Other';
    const pageData = params.get('pd');
    const eventData = params.get('ed');
    let requestType = 'Conversion';
      ...
      try {
        ...
        if (typeof data === 'object' && data !== null) {
          Object.entries(data).forEach(([key, data]) => {
            const result = super.handleQueryParam(`pd[${key}]`, data);
            if (result) {
              results.push(result);
            }
          });
        ...
        if (typeof data === 'object' && data !== null) {
          Object.entries(data).forEach(([key, data]) => {
            if (key === 'line_items') {
              ...
              const result = super.handleQueryParam(`ed[${key}]`, data);
              if (result) {
                results.push(result);
              }
            }
