...
class AdobeWebSdkProvider extends BaseProvider {
  constructor() {
    ...
    this._pattern =
      /\/ee\/.+[&?]configId=[0-9a-f]{8}\b-[0-9a-f]{4}\b-[0-9a-f]{4}\b-[0-9a-f]{4}\b-[0-9a-f]{12}/i;
  ...
  handleQueryParam(name, _value) {
    ...
    const eventMatch = name.match(/^events\[(\d+)]\./);
    ...
    return super.handleQueryParam(name, _value);
  }
  ...
  handleCustom(_url, _params) {
    ...
    const requestType = new URL(_url).pathname.split('/').pop();

    const eventNames = params
      .filter(([_key, _value]) => /\.eventType$/.test(_key))
