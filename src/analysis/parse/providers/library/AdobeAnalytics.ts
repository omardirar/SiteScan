...
class AdobeAnalyticsProvider extends BaseProvider {
  constructor() {
    ...
    this._pattern =
      /^([^#?]+)(\/b\/ss\/)|\.2o7\.net\/|\.sc\d?\.omtrdc\.net\/(?!id)/;
  ...
  parseUrl(rawUrl, postData = '') {
    ...
    for (const param of params) {
      ...
      const stackedParam = stacked.join('') + key,
  ...
  handleCustom(url, params) {
    const results = [];
    const rsid = url.pathname.match(/\/b\/ss\/([^/]+)\//);
    const jsVersion = url.pathname.match(/\/(JS-[^/]+)\//i);
    const pev2 = params.get('pe');
    ...
    if (jsVersion) {
      results.push({
        key: 'version',
        field: this.keys.version
          ? this.keys.version.name
          : 'JavaScript Version',
        value: jsVersion[1],
...
export default AdobeAnalyticsProvider;
