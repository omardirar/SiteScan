...
class PiwikPROTagManagerProvider extends BaseProvider {
   ...
   * @returns {void|Array}
   */
  handleCustom(url, _params) {
    const matches = url.pathname.match(/^\/containers\/([a-z0-9-]+)\.js/),
      id =
        matches && matches[1]
          ? matches[1]
          : /* istanbul ignore next: should never happen, but it's a simple string default */ '';
...
export default PiwikPROTagManagerProvider;
