...
class AdobeHeartbeatProvider extends BaseProvider {
  ...
  handleQueryParam(name, value) {
    ...
    if (/^customMetadata\./i.test(name)) {
      ...
      /^(playerTime|media\.player)?\./i.test(name) &&
      !(name in this.keys)
    ) {
      result = {
        key: name,
  ...
  handleCustom(_url, _params) {
    ...
    if (/\/api\/v1\/sessions\/?(([^/?#]+))?/.test(_url)) {
      // Media SDK v3
      event = _params.get('eventType') || '';
      account = _params.get('params.analytics.reportSuite');
      if (RegExp.$1) {
        results.push({
      ...
      // Media SDK v1/v2
      event = _params.get('s:event:type');
      account = _params.get('s:sc:rsid');
    }
...
export default AdobeHeartbeatProvider;
