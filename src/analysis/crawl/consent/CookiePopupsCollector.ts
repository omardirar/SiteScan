...
import type {
  AutoconsentAction,
  Collector,
  CookiePopupsCollectorOptions,
  CookiePopupsResult,
  CMPInfo,
  ScrapedFrame,
  CookiePopupsResultTiming,
} from './types.js';
...
export class CookiePopupsCollector implements Collector<CookiePopupsResult> {
  ...
  private startTs = 0;
  private actionTimestamp: number | undefined;
  ...
  async start(page: Page): Promise<void> {
    ...
    try {
      ...
      await Promise.all(
        frameIds.map(async (id) => {
          try {
            ...
            this.errors.push(
              `Frame isolated world failed: ${e?.message || String(e)}`,
            );
          }
        }),
      );
      ...
      this.errors.push(
        `Failed to build isolated worlds: ${e?.message || String(e)}`,
      );
    }
  }
  ...
  private async handleMessage(msg: any) {
    ...
    switch (msg.type) {
      ...
      case 'eval': {
        const session = this.cdpSessionsByContext.get(
          msg.executionContextUniqueId,
        );
        if (!session) break;
        let ok = false;
  ...
  private async evaluateInIsolated(
  ...
  private findMessages(partial: any): any[] {
    return this.receivedMsgs.filter((m) =>
      Object.keys(partial).every((k) => m[k] === partial[k]),
    );
  }
  ...
  private async scrapeFramesTimeboxed(): Promise<ScrapedFrame[]> {
    const result = await Promise.race([
      this.scrapeFrames(),
      new Promise<ScrapedFrame[]>((_, rej) =>
        setTimeout(
          () => rej(new Error('Scraping timed out')),
          this.options.scrapeTimeoutMs,
        ),
      ),
    ]).catch((e) => {
    ...
    const tasks = Array.from(this.cdpSessionsByContext.entries()).map(
      async ([ctx, session]) => {
        try {
          ...
          if (
            value.cleanedText ||
            (value.potentialPopups && value.potentialPopups.length > 0)
          )
            return value;
        } catch (e: any) {
  ...
  private collectCMPResults(): CMPInfo[] {
    ...
    for (const msg of detectedRules) {
      ...
      processed.push(cmp);
      const resultType =
        this.options.autoconsentAction === 'optOut'
          ? 'optOutResult'
          : 'optInResult';
      ...
      cmps.push({
        ...
        started: Boolean(
          this.options.autoconsentAction && found && found.length > 0,
        ),
        ...
        patterns: Array.from(
          new Set(
            this.findMessages({ type: 'report' }).flatMap(
              (r: any) => r.state?.heuristicPatterns || [],
            ),
          ),
        ),
        snippets: Array.from(
          new Set(
            this.findMessages({ type: 'report' }).flatMap(
              (r: any) => r.state?.heuristicSnippets || [],
            ),
          ),
        ),
        filterListMatched: Boolean(
          this.findMessages({ type: 'popupFound', cmp: 'filterList' }).length >
            0,
        ),
      });
    }
    if (cmps.length === 0) {
      ...
      const filterListMatched = Boolean(
        this.findMessages({ type: 'popupFound', cmp: 'filterList' }).length > 0,
      );
  ...
  async awaitResult(): Promise<CookiePopupsResult> {
    ...
    const detected = await this.waitForMessage(
      { type: 'cmpDetected' },
      this.options.detectTimeoutMs,
      200,
    );
    if (detected) {
      const found = await this.waitForMessage(
        { type: 'popupFound' },
        this.options.foundTimeoutMs,
        200,
      );
    ...
    const timing: CookiePopupsResultTiming = {
      totalMs: Date.now() - this.startTs,
    };
    if (this.actionTimestamp) {
      timing.actionTimestamp = this.actionTimestamp;
    }
...
const SCRAPE_SCRIPT = (function SCRAPE_SCRIPT_FACTORY() {
  const BUTTON_LIKE_ELEMENT_SELECTOR =
  ...
  function isDisabled(el: any) {
    return (
      ('disabled' in el && Boolean((el as any).disabled)) ||
  ...
  function getPopupLikeElements() {
    const walker = document.createTreeWalker(
      ...
      (window as any).NodeFilter.SHOW_ELEMENT,
      {
        acceptNode(node: any) {
          if ((node as any).tagName === 'BODY')
            return (window as any).NodeFilter.FILTER_SKIP;
          const cssPosition = (window as any).getComputedStyle(node).position;
          if (
            (cssPosition === 'fixed' || cssPosition === 'sticky') &&
            isVisible(node)
          )
            return (window as any).NodeFilter.FILTER_ACCEPT;
          return (window as any).NodeFilter.FILTER_SKIP;
        },
      } as any,
    );
    const found: any[] = [];
    for (
      let node = (walker as any).nextNode();
      node;
      node = (walker as any).nextNode()
    ) {
      found.push(node as any);
    }
  ...
  function getDocumentText() {
      ...
      while ((node = (walker as any).nextNode())) {
        ...
        for (const child of (element.shadowRoot as any).children) {
          if (
            ...
            !ELEMENT_TAGS_TO_SKIP.includes(child.tagName)
          ) {
    ...
    const visibleText = ((document.body ?? document.documentElement) as any)
      .innerText;
  ...
  function getSelector(el: any, specificity: any) {
    ...
    let parent;
    let result = '';
    if (element.nodeType !== (window as any).Node.ELEMENT_NODE) {
      return result;
    }
    ...
    while (parent instanceof HTMLElement) {
      ...
      if (
        specificity.dataAttributes &&
        element.attributes instanceof NamedNodeMap
      ) {
        const dataAttributes = Array.from(element.attributes).filter((a: any) =>
          a.name.startsWith('data-'),
        );
        dataAttributes.forEach((a: any) => {
          const escapedValue = (window as any).CSS.escape(a.value);
  ...
  function getUniqueSelector(el: any) {
    ...
    try {
      ...
      if (
        e &&
        e.message &&
        String(e.message).includes('is not a valid selector')
      )
  ...
  function getButtonData(el: any) {
    const actionableButtons = excludeContainers(
      getButtonLikeElements(el),
    ).filter((b: any) => isVisible(b) && !isDisabled(b) && b.innerText.trim());
    return actionableButtons.map((b: any) => ({
  ...
  function scrapePage() {
    const isFramed =
      window.top !== window || (location as any).ancestorOrigins?.length > 0;
    if (
      isFramed &&
      (window as any).parent &&
      (window as any).parent !== (window as any).top
    ) {
      return {
        isTop: !isFramed,
