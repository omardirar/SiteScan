import type { TrackerEvent } from '../../core/model/types.js';
import { providers as libraryProviders } from './library/registry';
...
function buildDetectorFromProvider(p: any): DetectorFn {
  return (url: string) => {
    try {
      ...
      const providerKey: string = String(res.provider.key || p._key || 'unknown');
      ...
      for (const d of res.data as any[]) {
        ...
        if ('field' in d && d.field != null) details[`field:${key}`] = String(d.field);
        if ('group' in d && d.group != null) details[`group:${key}`] = String(d.group);
